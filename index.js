...
sig.ensureVal = function (v) {
  return sig.isSig(v) ? v.then(sig.val()) : sig.val(v)
}
...
sig.any = function (values) {
  ...
  var t = s.filter(sig.spread, sig.isSig).each(sig.spread, function (u, k) {
...
sig.merge = function (values) {
  return sig.any(values).map(sig.spread, sig.identity)
}
...
sig.prototype.redir = sig.prototype.to = function (t) {
  return this.call(redirect, t).done().call(dependOn, t)
}
...
function setSource(t, s) {
  if (t.source)
    t.throw(
      new Error("Cannot set signal's source, signal already has a source")
    )
  else t.source = s
}
...
function redirect(s, t) {
  return this.each(sig.putTo, t).catch(function (e) {
    t.throw(e)
    this.next()
  })
}
...
function tapSig(s, t) {
  ...
  s.call(redirect, t).call(dependOn, u)
...
function objEach(obj, fn) {
  ...
  for (var k in obj) if (hasOwnProperty.call(obj, k)) fn(obj[k], k)
}
...
function copy(obj) {
  ...
  var result = {}
  for (var k in obj) if (hasOwnProperty.call(obj, k)) result[k] = obj[k]
  return result
}
...
function isArguments(obj) {
  return (
    typeof obj == 'object' && typeof obj.length == 'number' && 'callee' in obj
  )
}
